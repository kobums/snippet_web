...
export default function OcrButton({ onExtract, className = '' }: OcrButtonProps) {
  ...
  const buttonBase = `flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/60 border border-gray-200 dark:bg-white/5 dark:border-white/8 text-sm text-gray-600 dark:text-[#a0a0a0] hover:bg-white dark:hover:bg-white/10 transition-all disabled:opacity-50`;

  if (loading) {
    return (
      <button type="button" disabled className={`${buttonBase} ${className}`}>
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      ...
      <button
        ...
        title="이미지 파일에서 텍스트 추출 (OCR)"
        className={buttonBase}
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
        </svg>
        <span className="hidden sm:inline">이미지 OCR</span>
      </button>
    </div>
  );
}
