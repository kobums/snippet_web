...
export default function OwnedBooksPage() {
  ...
  const loadBooks = async () => {
    try {
      ...
      setBooks(allBooks.filter(b => b.type === 'have'));
    } catch (e) {
      handleApiError(e, '소장한 책을 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-[#f0f0f0]">소장한 책</h2>
