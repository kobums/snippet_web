...
export default function CompletedBooksPage() {
  ...
  const loadBooks = async () => {
    try {
      ...
      handleApiError(e, '완독한 책을 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-[#f0f0f0]">완독</h2>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 dark:text-[#666]">
          ...
          <p className="text-sm">{searchQuery ? '검색 결과가 없습니다' : '완독한 책이 없습니다'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
          {filteredBooks.map(book => (
            ...
            >
              {book.coverUrl ? (
                <div className="w-full aspect-[2/3] rounded-lg overflow-hidden mb-3 bg-gray-50 dark:bg-white/5 relative">
              ...
              {book.endDate && (
                <p className="text-[10px] text-gray-400 dark:text-[#666] mt-1.5">
                  {new Date(book.endDate).toLocaleDateString('ko-KR', { year: 'numeric', month: 'short', day: 'numeric' })}
                </p>
              )}
            </div>
          ))}
