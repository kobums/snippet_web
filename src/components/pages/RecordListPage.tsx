  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-[#f0f0f0]">{title}</h2>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-[#a0a0a0] mt-1">{description} · {records.length}개</p>
        </div>
          ...
          {filteredRecords.map(record => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 dark:text-[#666] shrink-0">
                  <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14.5 2 14.5 8 20 8"/>
                </svg>
                <span className="text-xs font-semibold text-gray-900 dark:text-[#f0f0f0] truncate">
                  {record.tag ? `@${record.tag}` : title}
                </span>
                <span className="text-[10px] text-gray-400 dark:text-[#666] ml-auto tabular-nums shrink-0">{formatDate(record.createDate)}</span>
              </div>
              ...
              <div className="border-t border-gray-100 dark:border-white/8 pt-3 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-gray-500 dark:text-[#a0a0a0] truncate group-hover:text-accent transition-colors">{record.bookTitle}</span>
                </div>

                {/* 페이지 */}
                <p className="text-xs text-gray-400 dark:text-[#666]">p.{record.relatedPage}</p>

                {/* 본문 */}
                <p className="text-sm text-gray-700 dark:text-[#d0d0d0] leading-relaxed whitespace-pre-wrap line-clamp-4">{record.text}</p>
              </div>
            </div>
          ))}
        </div>
