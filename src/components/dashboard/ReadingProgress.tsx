      ...
      <div className="bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-md relative z-10">
        ...
        <div className="flex items-center justify-between mb-5 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {showSortMenu && (
                <div className="absolute right-0 top-full mt-1 bg-[#1a1a2e]/95 border border-white/10 rounded-xl p-1 min-w-[120px] z-50 backdrop-blur-xl shadow-xl">
                  {([['newest', '최신순'], ['title', '제목순'], ['author', '작가순']] as const).map(([key, label]) => (
                    <button
                      key={key}
                      onClick={() => { setSortOption(key); setShowSortMenu(false); }}
                      className={`w-full text-left px-3 py-1.5 rounded-lg text-xs transition-colors ${
                        sortOption === key ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white/80'
                      }`}
                    >
                      {label}
                    </button>
        ...
        {loading ? (
          ...
          <div className="h-32 bg-white/5 border border-dashed border-white/20 rounded-2xl flex items-center justify-center flex-col gap-2">
            ...
            <button onClick={onNewClick} className="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg transition-colors mt-2">새로운 책 추가하기</button>
          </div>
        ) : (
          <div className="space-y-2">
            <div className="text-xs text-white/40 mb-3">할 일 {filteredBooks.length}</div>
            {filteredBooks.map(book => {
              const progressPercent = book.totalPage > 0 ? Math.round((book.readPage / book.totalPage) * 100) : 0;
              return (
                <div key={book.id} onClick={() => onBookClick(book)} className="bg-white/5 border border-white/10 p-4 rounded-2xl flex flex-col gap-3 group relative overflow-hidden cursor-pointer hover:bg-white/[0.07] transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {activeTab === 'reading' && (
                        <>
                          ...
                          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden cursor-pointer" onClick={(e) => onProgressChange(book.id, book.readPage, book.totalPage || 100, e)}>
                  ...
                  {activeTab === 'reading' && (
                    <div className="flex gap-2 relative z-10 mt-1">
                      <button onClick={(e) => onStatusChange(book.id, 'completed', e)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-white/70 text-xs rounded-lg transition-colors">완독 처리</button>
                      <button onClick={(e) => onStatusChange(book.id, 'dropped', e)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-white/70 text-xs rounded-lg transition-colors">중단</button>
                    </div>
                  )}
                  {activeTab === 'waiting' && (
                    <div className="flex gap-2 relative z-10 mt-1">
                      <button onClick={(e) => onStatusChange(book.id, 'reading', e)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-white/70 text-xs rounded-lg transition-colors">읽기 시작</button>
                    </div>
                  )}
