  ...
  return (
    <div className="mb-3 sm:mb-4 shrink-0 bg-white/70 border border-gray-200 rounded-2xl p-3 sm:p-4 flex flex-col gap-2.5 sm:gap-3 shadow-md animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      <select value={formBook} onChange={e => setFormBook(e.target.value ? Number(e.target.value) : '')}
        className="text-xs sm:text-sm bg-white/80 border border-gray-200 rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-gray-700 outline-none focus:border-blue-400 transition-colors cursor-pointer">
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <input type="text" value={formTag} onChange={e => setFormTag(e.target.value)}
            placeholder="태그 (선택)"
            className="w-full text-xs sm:text-sm bg-white/80 border border-gray-200 rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-gray-700 outline-none focus:border-blue-400 transition-colors placeholder-gray-400" />
        </div>
        <div>
          <input type="number" value={formPage} onChange={e => setFormPage(e.target.value ? Number(e.target.value) : '')}
            placeholder="페이지"
            className="w-20 sm:w-24 text-xs sm:text-sm bg-white/80 border border-gray-200 rounded-lg px-2.5 sm:px-3 py-1.5 sm:py-2 text-gray-700 outline-none focus:border-blue-400 transition-colors placeholder-gray-400" />
        </div>
      </div>
