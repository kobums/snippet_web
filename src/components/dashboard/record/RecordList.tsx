...
export default function RecordList({ records, loading, onOpenForm, onRecordClick }: RecordListProps) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 sm:gap-4 py-8 sm:py-10">
        ...
        <div className="text-center">
          <p className="text-gray-400 text-sm sm:text-base">작성된 기록이 없습니다.</p>
          <button
            onClick={onOpenForm}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-2.5">
      {records.map(record => (
        <div key={record.id} onClick={() => onRecordClick?.(record.bookId)} className="bg-white/60 border border-gray-200 rounded-2xl px-3 sm:px-3.5 py-2.5 sm:py-3 flex flex-col shadow-sm hover:bg-white transition-colors cursor-pointer group">
          {/* 헤더: 태그 (제목) + 날짜 */}
          <div className="flex items-center gap-2 mb-2 sm:mb-2.5">
            ...
            <span className="text-[10px] sm:text-xs text-gray-400 ml-auto tabular-nums shrink-0">{formatDate(record.createDate)}</span>
          </div>
          ...
          <div className="border-t border-gray-100 pt-2 flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs sm:text-sm text-gray-500 truncate">{record.bookTitle}</span>
            </div>

            {/* 읽은 부분 */}
            <p className="text-xs sm:text-sm text-gray-500">p.{record.relatedPage}</p>

            {/* 본문 */}
            <p className="text-xs sm:text-sm text-gray-700 leading-relaxed whitespace-pre-wrap line-clamp-2">{record.text}</p>
          </div>
        </div>
      ))}
    </div>
