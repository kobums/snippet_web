  ...
  return (
    <div className="liquid-panel p-4 sm:p-5 md:p-6 flex flex-col shrink-0">
      <div className="flex items-center justify-between mb-4 sm:mb-5 shrink-0">
        ...
        <span className="text-xs sm:text-sm text-gray-400 dark:text-[#666]">{completedBooks.length}권</span>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:gap-4 overflow-y-auto hide-scrollbar flex-1">
        {loading ? (
          ...
          <div className="text-gray-400 dark:text-[#666] text-sm sm:text-base py-6 sm:py-8 text-center border border-dashed border-gray-200 rounded-2xl">
            아직 다 읽은 책이 없습니다.
          </div>
        ) : (
            ...
            >
              <div className="w-full h-[180px] sm:h-[200px] bg-gray-50 dark:bg-white/5 flex items-center justify-center p-3 sm:p-4">
                {book.coverUrl ? (
                  ...
                  <div className="w-16 h-24 bg-gray-200 rounded-md flex items-center justify-center text-[10px] text-gray-400 dark:text-[#666]">No Image</div>
                )}
              </div>
              <div className="px-3 sm:px-4 py-2.5 sm:py-3 flex flex-col gap-1.5">
                <h4 className="flex items-center gap-2 text-gray-900 dark:text-[#f0f0f0] font-medium text-sm sm:text-base leading-snug group-hover:text-accent transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 dark:text-[#666] shrink-0">
                    <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
                  </svg>
                  <span className="truncate">{book.title}</span>
                </h4>
                <p className="text-xs sm:text-sm text-gray-400 dark:text-[#666] pl-5">{book.author}</p>
                <div className="flex items-center gap-1.5 pl-5 mt-1">
                  ...
                  <span className="text-[11px] sm:text-xs text-gray-400 dark:text-[#666]">{formatDate(book.startDate)} ~ {formatDate(book.endDate)}</span>
                </div>
              </div>
            </div>
          ))
