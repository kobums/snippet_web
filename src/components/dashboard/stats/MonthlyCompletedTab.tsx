      ...
      <div className="grid grid-cols-12 gap-0.5 sm:gap-1 mb-2">
        {Object.entries(monthlyStats).map(([month, count]) => (
          <div key={month} className="flex flex-col items-center gap-1">
            ...
            <span className="text-[9px] sm:text-[10px] text-gray-400">{month}월</span>
            <span className="text-[10px] sm:text-xs font-medium text-gray-700">{count}</span>
          </div>
        ))}
      </div>
      <div className="border-t border-gray-100 pt-2.5 sm:pt-3 flex flex-col gap-1.5 sm:gap-2">
        {Object.entries(monthlyStats)
          .filter(([, count]) => count > 0)
          .map(([month, count]) => (
            <div key={month} className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
              <span className="text-gray-400 w-8 sm:w-10 shrink-0">{month}월</span>
              <div className="flex-1 bg-gray-100 rounded-full h-1.5">
                <div className="h-full rounded-full bg-[#5ebd8a] transition-all"
                  style={{ width: `${(count / maxMonthly) * 100}%` }} />
              </div>
              <span className="text-gray-700 font-medium w-5 sm:w-6 text-right">{count}</span>
            </div>
          ))}
      </div>
    </div>
