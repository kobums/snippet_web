    ...
    <div className="flex flex-col gap-5 sm:gap-6">
      ...
      <div>
        <button onClick={() => setReadingExpanded(p => !p)}
          ...
          <span className="text-gray-400 dark:text-[#666]">{readingBooks.length}</span>
        </button>

        {readingExpanded && (
          <div className="flex flex-col gap-1.5 pl-1">
            {readingBooks.length === 0 ? (
              <p className="text-xs sm:text-sm text-gray-400 dark:text-[#666] pl-2">진행 중인 책이 없습니다.</p>
            ) : (
              readingBooks.map(b => (
      ...
      <div>
        <button onClick={() => setCompletedExpanded(p => !p)}
          ...
          <span className="text-gray-400 dark:text-[#666]">{completedBooks.length}</span>
        </button>

        {completedExpanded && (
          <div className="flex flex-col gap-1.5 pl-1">
            {completedBooks.length === 0 ? (
              <p className="text-xs sm:text-sm text-gray-400 dark:text-[#666] pl-2">완료된 책이 없습니다.</p>
            ) : (
      ...
      {/* 설명 */}
      <div className="flex flex-col gap-1 pt-2 border-t border-gray-100 dark:border-white/8">
        <p className="text-[11px] sm:text-xs text-gray-400 dark:text-[#666] flex gap-2">
          <span>💡</span>
  ...
  return (
    <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-[#a0a0a0] hover:text-gray-900 dark:text-[#f0f0f0] hover:bg-white dark:hover:bg-white/10/40 rounded-lg px-2 py-1 transition-all cursor-pointer group">
      ...
      <span className="text-gray-500 dark:text-[#a0a0a0] shrink-0">
        《<span className="text-gray-800 dark:text-[#d0d0d0] group-hover:text-accent transition-colors">{book.title}</span>》
      </span>
      <span className="text-gray-400 dark:text-[#666]">읽기</span>
    </div>
  );
}
