...
export default function YearlyComparisonSection({ data }: YearlyComparisonSectionProps) {
  ...
  if (isMobile) {
    return (
      <div className="liquid-panel p-5 space-y-4">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-[#d0d0d0]">연도별 비교</h2>

        <div className="space-y-3">
          {data.map((yearData) => (
            <div key={yearData.year} className="liquid-card p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-semibold text-lg text-gray-700 dark:text-[#d0d0d0]">{yearData.year}년</span>
  ...
  return (
    <div className="liquid-panel p-5 md:p-6 space-y-6">
      <h2 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-[#d0d0d0]">연도별 비교</h2>
