  ...
  return (
    <div className="liquid-panel p-4 sm:p-5 md:p-6 shrink-0">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        ...
        <span className="text-xs sm:text-sm text-gray-400 dark:text-[#666]">{viewYear}년 {viewMonth + 1}월</span>
      </div>

      <div className="grid grid-cols-7 gap-1 sm:gap-1.5 text-center text-[10px] sm:text-xs font-medium text-gray-400 dark:text-[#666] mb-1.5 sm:mb-2">
      ...
      <div className="grid grid-cols-7 gap-1 sm:gap-1.5">
        ...
        {days.map(day => {
          ...
          return (
            <div
              key={day}
              className={`aspect-[3/4] relative group rounded-md overflow-hidden border ${isToday ? 'border-accent/30 bg-accent/5' : 'border-gray-100 dark:border-white/8 bg-gray-50/50 dark:bg-white/2'} flex items-center justify-center transition-all ${hasBooks ? 'cursor-pointer hover:border-accent/30 shadow-sm' : ''}`}
            ...
            >
              {hasBooks ? (
                ...
                <span className={`text-[10px] ${isToday ? 'text-accent font-bold' : 'text-gray-400 dark:text-[#666]'}`}>{day}</span>
              )}
            </div>
          );
        })}
